...
async def google_login(
    ...
    # Verify Google token
    try:
        info = id_token.verify_oauth2_token(auth.id_token, google_requests.Request(), settings.GOOGLE_CLIENT_ID)
    ...
    await create_session(db, user.user_id, refresh_token, expires_at)
...
async def refresh_access_token(
    ...
    # Optional: rotate refresh token? We keep same for simplicity.
    return TokenRefresh(
...
async def get_session(
    ...
    # Same logic as refresh but also returns user info
    refresh_token = request.cookies.get("refresh_token")
    if not refresh_token:
        raise HTTPException(status_code=401, detail="session_expired")
